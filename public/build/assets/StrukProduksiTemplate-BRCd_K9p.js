var e=(e=new Date)=>new Intl.DateTimeFormat(`id-ID`,{day:`2-digit`,month:`short`,year:`numeric`}).format(e),t=(e=new Date)=>new Intl.DateTimeFormat(`id-ID`,{hour:`2-digit`,minute:`2-digit`,hour12:!1}).format(e),n=e=>{if(!e)return null;if(typeof e==`string`){let t=e.match(/^(\d{2})-(\d{2})-(\d{4})$/);if(t){let[,e,n,r]=t;return new Date(Number(r),Number(n)-1,Number(e))}}let t=new Date(e);return Number.isNaN(t.getTime())?null:t},r=(e=new Date)=>{let t=e instanceof Date?e:n(e);return t?new Intl.DateTimeFormat(`id-ID`,{day:`2-digit`,month:`2-digit`,year:`numeric`}).format(t).replace(/\//g,`-`):e||`-`},i=e=>Math.round(Number(e||0)).toLocaleString(`id-ID`),a=`
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .sub { font-size: 11px; font-weight: 700; letter-spacing: 2px; margin-top: 1px; }
    .contact { font-size: 14px; font-weight: 700; margin-top: 2px; letter-spacing: 0; }
    .topline { width: 100%; border-collapse: collapse; table-layout: fixed; margin-top: 2px; font-size: 13px; }
    .topline td { border: none; padding: 3px 0 4px; white-space: nowrap; }
    .topline .invoice { font-size: 12px; }
    .topline .amount { padding-right: 1mm; text-align: right; }
    .invoice { font-size: 14px; font-weight: 800; }
    .title { font-size: 14px; font-weight: 800; margin-bottom: 2px letter-spacing: 1px; text-align: center; }
    .block { margin-top: 1px; }
    .label { display: block; font-size: 12px; }
    .name { font-size: 15px; margin-top: 2px; }
    .address { min-height: 18px; overflow-wrap: anywhere; font-size: 13px; }
    .pay-row { display: grid; grid-template-columns: 1fr 1fr; margin: 5px 0 3px; text-align: center; font-size: 13px; }
    ...
    th { border-top: 2px solid #000; border-bottom: 2px solid #000; font-weight: 900; text-align: center; font-size: 13px; }
    td { padding-top: 1px; padding-bottom: 1px; }
    .bahan { width: 17mm; text-align: left; }
    .ukuran-col { width: 14mm; text-align: left; font-size: 12px; font-weight: bold; white-space: nowrap; }
    .ukuran { display: block; font-size: 11px; color: #444; margin-top: 0; }
    .qty { width: 7mm; text-align: center; font-variant-numeric: tabular-nums; font-size: 13px; }
    .amount { width: 18mm; text-align: right; font-variant-numeric: tabular-nums; white-space: nowrap; font-size: 13px; }
    .alt td { background: #eee; }
    .muted { color: #000; }
    .keterangan-row td { padding-top: 0; padding-bottom: 1px; font-size: 12px; }
    tfoot td { padding: 1px 2px; white-space: nowrap; }
    .summary-amount { width: mm; text-align: right; font-variant-numeric: tabular-nums; font-size: 13px; }
    ...
    .notes li { font-size: 11px; line-height: 1; margin-bottom: 0; padding: 1px 4mm; }
    .notes li::before { content: '* '; margin-left: -4mm; }
    .signatures { display: flex; justify-content: space-between; margin-top: 7px; text-align: center; font-size: 13px; }
    .signatures > div { width: 48%; }
    .sign-name { margin-top: 28px; min-height: 14px; font-size: 14px; font-weight: bold; }
    .printed { display: flex; justify-content: space-between; margin-top: 3px; font-size: 11px; }
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c,minimumHarga:l=0,hargaAkhirInvoice:u=null})=>{let d=new Date,f=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let p=n[0],m=p?.customer,h=p?.no_invoice||p?.kode_spk||`-`;p?.tgl_kirim;let g=n[0]?.cs?.username||o?.user?.username||o?.user?.name||`Admin`,_={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,v=c&&Number(c.diskon||0)>0,y=v?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,b=v?Number(c.harga_diskon||f):f,x=Number(l)>0,S=u?Number(u):b+Number(l),C=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
                ...
                <td class="amount" style="font-size: 13px; font-weight: bold;">${i(e.total_harga)}</td>
            </tr>
...
<body>
    <div class="receipt">
        ...
        <table class="topline">
            <tr>
                <td class="bahan invoice" style="font-weight: bold; font-size: 14px">${h}</td>
        ...
        <div class="title" style="font-size: 17px"><b>FAKTUR</b></div>
        <div class="block" style="font-size:14px; font-weight: bold;">
