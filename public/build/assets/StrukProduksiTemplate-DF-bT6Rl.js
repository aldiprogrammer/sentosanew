...
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c,minimumHarga:l=0,hargaAkhirInvoice:u=null,uang:d=null,kembalian:f=null,adaInvoice:p=!1})=>{let m=new Date,h=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let g=n[0],_=g?.customer,v=g?.no_invoice||g?.kode_spk||`-`,y=p&&g?.tanggal?r(g.tanggal):r();g?.tgl_kirim;let b=o?.user?.role===`Customer Service`?o?.user?.username||o?.user?.name||`Admin`:n[0]?.cs?.username||o?.user?.username||o?.user?.name||`Admin`,x={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,S=f===null?null:Math.max(0,Number(f)),C=c&&Number(c.diskon||0)>0,w=C?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,T=C?Number(c.harga_diskon||h):h,E=Number(l)>0,D=u?Number(u):T+Number(l),O=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
                <td class="bahan">${e.bahan?.kode||`-`}</td>
                <td class="ukuran-col" style="text-align: center">${e.lebar||0}x${e.tinggi||0}${e.satuan?` `+e.satuan:``}</td>
...
<body>
    <div class="receipt">
        ...
        <table class="topline">
            <tr>
                <td class="bahan invoice" style="font-weight: bold;">${v}</td>
                <td class="ukuran-col"></td>
                <td class="qty"></td>
                <td class="amount" style="font-weight: bold;">${y}</td>
            </tr>
        </table>
        ...
        <div class="block" style="font-weight: bold;">
            ...
            <div class="name">${_?.nama||`-`}</div>
            <div class="address">${_?.alamat||`-`}</div>
        ...
        <table>
            ...
            <tfoot>
                <tr><td class="summary-label" colspan="3">Total Faktur</td><td class="summary-amount" colspan='1' style="font-weight: bold">${i(h)}</td></tr>
                ${C?`<tr><td class="summary-label" colspan="3">Diskon (${w})</td><td colspan='1' class="summary-amount" style="color:#c00">-${i(h-T)}</td></tr>`:``}
                ${E?`<tr><td class="summary-label" colspan="3">Minimum Faktur</td><td class="summary-amount" style="color:#c00">+${i(l)}</td></tr>`:``}
                ${C||E?`<tr><td class="summary-label" colspan="3" style="font-weight:900">Harga Akhir</td><td colspan='1' class="summary-amount" style="font-weight:900">${i(D)}</td></tr>`:``}
            </tfoot>
        </table>
        ...
        <table>
            <tfoot>
                ...
                <tr><td class="summary-label" colspan="3">Kembalian</td><td class="summary-amount">${i(S)}</td></tr>
            </tfoot>
        </table>`}
...
<br>
        ...
        <div class="signatures" style="margin-top: 10px; font-weight: bold;">
            <div>
                <div>Hormat Kami,</div>
                <div class="sign-name">${b}</div>
            </div>
            <div>
                <div>Customer</div>
                <div class="sign-name">${_?.nama||`-`}</div>
            </div>
        </div>

        <div class="printed" style="margin-top: 7px;">
            <span>Printed By : ${b}</span>
            <span>${e(m)} ${t(m)}</span>
        </div>
    </div>
