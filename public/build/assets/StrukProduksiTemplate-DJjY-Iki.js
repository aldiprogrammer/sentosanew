...
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c,minimumHarga:l=0,hargaAkhirInvoice:u=null})=>{let d=new Date,f=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let p=n[0],m=p?.customer,h=p?.no_invoice||p?.kode_spk||`-`;p?.tgl_kirim;let g=n[0]?.cs?.username||o?.user?.username||o?.user?.name||`Admin`,_={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,v=c&&Number(c.diskon||0)>0,y=v?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,b=v?Number(c.harga_diskon||f):f,x=Number(l)>0,S=u?Number(u):b+Number(l),C=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
                <td class="bahan">
                    ${e.bahan?.kode||`-`}
                    <span class="ukuran" style="font-size:13px; font-weight: bold;">${e.lebar||0} x ${e.tinggi||0} ${e.satuan||``}</span>
                </td>
    ...
    <div class="receipt">
        ...
        <table class="topline">
            <tr>
                ...
                <td class="qty"></td>
                <td class="amount" style="font-weight: bold; font-size: 12px">${r(d)}</td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tfoot>
                ...
                ${v?`<tr><td class="summary-label" colspan="1">Diskon (${y})</td><td colspan='2' class="summary-amount" style="color:#c00">-${i(f-b)}</td></tr>`:``}
                ${x?`<tr><td class="summary-label" colspan="2">Minimum Faktur</td><td class="summary-amount" style="color:#c00">+${i(l)}</td></tr>`:``}
                ${v||x?`<tr><td class="summary-label" colspan="1" style="font-weight:900">Harga Akhir</td><td colspan='2' class="summary-amount" style="font-weight:900">${i(S)}</td></tr>`:``}
            </tfoot>
        </table>
