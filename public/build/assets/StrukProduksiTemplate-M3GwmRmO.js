...
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c,minimumHarga:l=0,hargaAkhirInvoice:u=null})=>{let d=new Date,f=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let p=n[0],m=p?.customer,h=p?.no_invoice||p?.kode_spk||`-`,g=p?.tgl_kirim||d,_=o?.user?.username||o?.user?.name||`Admin`,v={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,y=c&&Number(c.diskon||0)>0,b=y?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,x=y?Number(c.harga_diskon||f):f,S=Number(l)>0,C=u?Number(u):x+Number(l),w=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
...
<body>
    <div class="receipt">
        ...
        <table>
            ...
            <tfoot>
                <tr styele="border: 1px solid black"><td class="summary-label" colspan="3">Total Faktur</td><td class="amount" style="font-weight: bold; font-size: 11px">${i(f)}</td></tr>
                ${y?`<tr><td class="summary-label" colspan="3">Diskon (${b})</td><td class="amount" style="color:#c00">-${i(f-x)}</td></tr>`:``}
                ${S?`<tr><td class="summary-label" colspan="3">Minimum Faktur</td><td class="amount" style="color:#c00">+${i(l)}</td></tr>`:``}
                ${y||S?`<tr><td class="summary-label" colspan="3" style="font-weight:900">Harga Akhir</td><td class="amount" style="font-weight:900">${i(C)}</td></tr>`:``}
            </tfoot>
        </table>
