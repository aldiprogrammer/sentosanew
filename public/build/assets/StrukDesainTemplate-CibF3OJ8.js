...
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c,uang:l=null,kembalian:u=null})=>{let d=new Date,f=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let p=n[0],m=p?.customer,h=p?.no_invoice||p?.no_antrian||`-`,g=p?.invoice_desain&&p?.tanggal?r(p.tanggal):r(),_=u===null?null:Math.max(0,Number(u)),v=n[0]?.cs?.username||o?.user?.username||o?.user?.name||`Admin`,y={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,b=c&&Number(c.diskon||0)>0,x=b?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,S=b?Number(c.harga_diskon||f):f,C=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
    ...
    <div class="receipt">
        ...
        <table class="topline">
            <tr>
                ...
                <td class="ukuran"></td>
                <td class="qty"></td>
                <td class="amount" style="font-weight: bold; font-size: 12px">${g}</td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tfoot>
                ...
                ${b?`<tr><td class="summary-label" colspan="3">Diskon (${x})</td><td class="summary-amount" style="color:#c00">-${i(f-S)}</td></tr>`:``}
                ${b?`<tr><td class="summary-label" colspan="3" style="font-weight:900">Harga Akhir</td><td class="summary-amount" style="font-weight:900">${i(S)}</td></tr>`:``}
            </tfoot>
        </table>
        ...
        <table>
            <tfoot>
                ...
                <tr><td class="summary-label" colspan="3">Kembalian</td><td class="amount">${i(_)}</td></tr>
            </tfoot>
        </table>`}
