...
`,o=({items:n,auth:o,paymentType:s,diskonInfo:c})=>{let l=new Date,u=n.reduce((e,t)=>e+Number(t.total_harga||0),0);n.reduce((e,t)=>e+Number(t.qty||0),0);let d=n[0],f=d?.customer,p=d?.no_invoice||d?.kode_spk||`-`,m=d?.tgl_kirim||l,h=o?.user?.username||o?.user?.name||`Admin`,g={lunas:`TUNAI`,utang:`UTANG`,transfer:`TRANSFER`,qris:`QRIS`}[s]||`TUNAI`,_=c&&Number(c.diskon||0)>0,v=_?c.mode_diskon===`persen`?`${c.diskon}%`:`Rp ${i(c.diskon)}`:``,y=_?Number(c.harga_diskon||u):u,b=n.map((e,t)=>`
            <tr${t%2==1?` class="alt"`:``}>
...
<body>
    <div class="receipt">
        ...
        <table class="topline">
            <tr>
                <td class="bahan invoice">${p}</td>
                <td class="ukuran"></td>
                <td class="qty"></td>
                <td class="amount" style="font-weight: bold; font-size: 10px">${r(l)}</td>
            </tr>
        </table>
        ...
        <div class="block">
            ...
            <div class="name">${f?.nama||`-`}</div>
            <div class="address">${f?.alamat||`-`}</div>
        </div>
        <div>Selesai : ${r(m)}</div>
        <div>
        ...
        <table>
            ...
            <tfoot>
                <tr styele="border: 1px solid black"><td class="summary-label" colspan="3">Total Faktur</td><td class="amount" style="font-weight: bold; font-size: 11px">${i(u)}</td></tr>
                ${_?`<tr><td class="summary-label" colspan="3">Diskon (${v})</td><td class="amount" style="color:#c00">-${i(u-y)}</td></tr>`:``}
                ${_?`<tr><td class="summary-label" colspan="3" style="font-weight:900">Harga Akhir</td><td class="amount" style="font-weight:900">${i(y)}</td></tr>`:``}
        ...
        <div class="signatures" style="margin-top: 10px; font-weight: bold;">
            <div>
                <div>Hormat Kami,</div>
                <div class="sign-name">${h}</div>
            </div>
            <div>
                <div>Customer</div>
                <div class="sign-name">${f?.nama||`-`}</div>
            </div>
        </div>

        <div class="printed" style="margin-top: 7px;">
            <span>Printed By : ${h}</span>
            <span>${e(l)} ${t(l)}</span>
        </div>
    </div>
