...
export default function TransaksiBank({ transaksi, rekenings, kode, saldo }) {
  ...
  const buildFilters = () => {
    ...
    if (idRekening) params.id_nomor_rekening = idRekening;
    if (bulan) {
      params.bulan = bulan;
    } else {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      router.get('/transaksi-bank', buildFilters(), { preserveState: true, replace: true });
    }, 500);
    return () => clearTimeout(t);
  }, [search, tglDari, tglSampai, bulan, idRekening]);
  ...
  const resetFilters = () => {
    ...
    setBulan('');
    setIdRekening('');
    router.get('/transaksi-bank', {}, { preserveState: true, replace: true });
  };
  ...
  const openModalEdit = (item) => {
    ...
    setData({
      id: item.id,
      id_nomor_rekening: item.id_nomor_rekening?.toString() || '',
    ...
    <AdminLayout>
          ...
          <div className="card bg-base-100 shadow-md border border-base-300">
            <div className="card-body">
              <h2 className="card-title text-sm">
                Saldo {idRekening ? 'Rekening Terpilih' : 'Bank Saat Ini'}
              </h2>
        ...
        <div className="card bg-base-100 shadow-md border border-base-300">
          <div className="card-body">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-2">
              <h2 className="card-title">Transaksi Bank</h2>
            ...
            <div className="flex flex-col md:flex-row md:items-end gap-3 mb-3">
              <label className="form-control">
                <div className="label"><span className="label-text text-xs">Cari</span></div>
                <input
                  type="text"
                  placeholder="Cari kode / jenis biaya..."
              ...
              <label className="form-control">
                <div className="label"><span className="label-text text-xs">Bank / Rekening</span></div>
                <select
                  className="select select-bordered select-success select-sm"
                  value={idRekening}
                  onChange={(e) => setIdRekening(e.target.value)}
                >
      ...
      <dialog ref={modalRef} className="modal">
        <div className="modal-box max-w-lg">
          ...
          <h3 className="text-lg font-bold mb-4">Tambah Transaksi Bank</h3>
          <form onSubmit={save}>
            <div className="grid grid-cols-1 gap-3">
              <label className="form-control">
                <div className="label"><span className="label-text">Bank / Rekening</span></div>
                <select value={data.id_nomor_rekening} className="select select-bordered select-success w-full" required onChange={(e) => setData('id_nomor_rekening', e.target.value)}>
                  <option value="">-- Pilih Rekening --</option>
                  {rekenings.map((r) => (
                    <option key={r.id} value={r.id}>{r.nama_bank} - {r.nomor_rekening} (a.n. {r.atas_nama})</option>
                  ))}
                </select>
              </label>
      ...
      <dialog ref={editmodalRef} className="modal">
        <div className="modal-box max-w-lg">
          ...
          <h3 className="text-lg font-bold mb-4">Edit Transaksi Bank</h3>
