...
export default function Kurir({ kurir, kode }) {
  ...
  const exportPDF = () => {
    try {
      const doc = new jsPDF();
      doc.setFontSize(16);
      doc.text("Data Kurir", 14, 20);
      doc.setFontSize(10);
      doc.text("Tanggal: " + new Date().toLocaleDateString("id-ID"), 14, 27);
      const rows = kurir.map((item, index) => [index + 1, item.kode, item.nama, formatPhone(item.nohp)]);
      autoTable(doc, { startY: 32, head: [["No", "Kode", "Nama", "No Hp"]], body: rows, styles: { fontSize: 10 }, headStyles: { fillColor: [22, 163, 74] }, theme: "grid" });
      doc.save("data_kurir.pdf");
    } catch (error) {
