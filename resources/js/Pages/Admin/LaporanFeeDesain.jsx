...
export default function LaporanFeeDesain({
    ...
    const handleAmbilFee = (ids) => {
        ...
        setProcessing(true)
        router.put(route('laporan-fee-desain.ambil'), { ids }, {
            preserveScroll: true,
            onSuccess: () => {
                setSelected([])
                setProcessing(false)
    ...
    const handleAmbilSemua = () => {
        if (processing) return
        setProcessing(true)
        router.put(route('laporan-fee-desain.ambil'), { pengguna_id: filterPegawai || auth?.user?.id }, {
            preserveScroll: true,
            ...
            <div className="grid grid-cols-1 gap-4">
                <div className="card bg-base-100 shadow-md border border-base-300">
                    <div className="card-body">
                        <h2 className="card-title">Laporan Fee Desain</h2>
                                ...
                                <tbody>
                                    {transaksis.data.map((item, i) => (
                                        <tr key={item.id} className={item.status === 'belum_diambil' ? 'bg-yellow-50' : 'bg-green-50'}>
                                            ...
                                            <td className="font-semibold">Rp {Number(item.fee).toLocaleString('id-ID')}</td>
                                            {!isDesainer && <td>{item.pengguna?.username || '-'}</td>}
                                            <td>
                                    ...
                                    {transaksis.data.length === 0 && (
                                        <tr>
                                            <td colSpan={isDesainer ? 9 : 11} className="text-center text-gray-400 py-4">Tidak ada data</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
